...
// not ready, however this should be implemnented especially for
// viewing methods
const ContainerAccordion = () => {
  return (
    ...
    >
      ...
      <AccordionDetails>
        <TextField multiline fullWidth sx={{ pl: 2 }} />
      </AccordionDetails>
    </Accordion>
  );
};
...
const CreateMethod = () => {
  ...
  const { data: methodData, mutate: methodMutate } = useSWR(
    SERVER_BASE + `method/${location.state.methodId}`,
    fetcher
  );

  const { data: categoryData, mutate: categoryMutate } = useSWR(
    SERVER_BASE + `user/${user.id}/categories-no-methods`,
    fetcher
  );
  ...
  useEffect(() => {
    setState(methodData ? methodData : initialState);
    if (!(categoryData == undefined) && !(methodData == undefined)) {
      const category = categoryData.filter((category: any) =>
        category.id ? category.id == methodData.category_id : false
      );
      setSelectDisplay(category[0].name);
    }
    return () => {};
  }, [categoryData, methodData]);
